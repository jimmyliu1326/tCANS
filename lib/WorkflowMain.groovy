//
// This file holds several functions specific to the main.nf workflow in the nf-core/tCANS pipeline
//
...
class WorkflowMain {
    ...
    public static String help(workflow, params, log) {
        def command = "nextflow run jimmyliu1326/tCANS -r [version] --input samplesheet.csv --reference reference.fa --primers primers.bed"
        def help_string = ''
    ...
    public static void initialise(workflow, params, log, schema_filename='nextflow_schema.json') {
        ...
        if ( !Utils.fileExists(params.primers) ) {
            log.error "Please provide a valid path to the primers scheme"
            System.exit(1)
        }

        if ( !Utils.fileExists(params.reference) ) {
            log.error "Please provide a valid path to the reference sequence"
            System.exit(1)
        }
    }
}
